...
class CRNN(CNNBaseModel):
    ...
    def __feature_sequence_extraction(self, inputdata):
        ...
        conv5 = self.separable_conv2d(inputdata=max_pool4, out_channel=512, kernel_size=3, stride=1, name='conv5')  # batch*4*25*512
        relu5 = self.relu(conv5)  # batch*4*25*512
        bn5 = self.layerbn(inputdata=relu5, is_training=is_training)  # batch*4*25*512
        conv6 = self.separable_conv2d(inputdata=bn5, out_channel=512, kernel_size=3, stride=1, name='conv6')  # batch*4*25*512
        relu6 = self.relu(conv6)  # batch*4*25*512
        bn6 = self.layerbn(inputdata=relu6, is_training=is_training)  # batch*4*25*512
        max_pool6 = self.maxpooling(inputdata=bn6, kernel_size=[2, 1], stride=[2, 1])  # batch*2*25*512
        conv7 = self.conv2d(inputdata=max_pool6, out_channel=512, kernel_size=2, stride=[2, 1], name='conv7')  # batch*1*25*512
        relu7 = self.relu(conv7)  # batch*1*25*512
        return relu7

    def __map_to_sequence(self, inputdata):
        """
        Implement the map to sequence part of the network mainly used to convert the cnn feature map
